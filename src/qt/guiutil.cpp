...
#include <qt/synorixaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Synorix address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new SynorixAddressEntryValidator(parent));
    widget->setCheckValidator(new SynorixAddressCheckValidator(parent));
}
...
bool parseSynorixURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no synorix: URI
    if(!uri.isValid() || uri.scheme() != QString("synorix"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!SynorixUnits::parse(SynorixUnit::SNRX, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseSynorixURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseSynorixURI(uriInstance, out);
}

QString formatSynorixURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("synorix:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SynorixUnits::format(SynorixUnit::SNRX, info.amount, false, SynorixUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openSynorixConf()
{
    ...
    /* Open synorix.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Synorix.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Synorix (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Synorix (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Synorix*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "synorix.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("synorix-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a synorix.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Synorix\n";
        else
            optionFile << strprintf("Name=Synorix (%s)\n", ChainTypeToString(chain));
