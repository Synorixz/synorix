...
/** Call node.updateRwSetting() with Synorix 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Synorix 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/synorix/synorix/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because synorix 22.x will not try to read these.
...
//! Convert enabled/size values to synorix -prune setting.
...
//! Get pruning enabled value to show in GUI from synorix -prune setting.
...
//! Get pruning size value to show in GUI from synorix -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplaySynorixUnit")) {
        settings.setValue("DisplaySynorixUnit", QVariant::fromValue(SynorixUnit::SNRX));
    }
    QVariant unit = settings.value("DisplaySynorixUnit");
    if (unit.canConvert<SynorixUnit>()) {
        m_display_synorix_unit = unit.value<SynorixUnit>();
    } else {
        m_display_synorix_unit = SynorixUnit::SNRX;
        settings.setValue("DisplaySynorixUnit", QVariant::fromValue(m_display_synorix_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if synorix.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_synorix_unit);
...
{
    if (new_unit.isNull() || new_unit.value<SynorixUnit>() == m_display_synorix_unit) return;
    m_display_synorix_unit = new_unit.value<SynorixUnit>();
    QSettings settings;
    settings.setValue("DisplaySynorixUnit", QVariant::fromValue(m_display_synorix_unit));
    Q_EMIT displayUnitChanged(m_display_synorix_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/synorix/synorix/pull/8273
    ...
    // (https://github.com/synorix-core/gui/issues/567).
