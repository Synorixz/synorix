...
#include <synorix-build-config.h> // IWYU pragma: keep

#include <qt/synorixgui.h>
 ...
 *
 * Ref: https://github.com/synorix/synorix/pull/1026
 */
static constexpr int64_t MAX_BLOCK_TIME_GAP = 90 * 60;

const std::string SynorixGUI::DEFAULT_UIPLATFORM =
...
SynorixGUI::SynorixGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &SynorixGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &SynorixGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &SynorixGUI::showModalOverlay);
...
void SynorixGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Synorix address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and synorix: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &SynorixGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &SynorixGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &SynorixGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Synorix addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Synorix addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Synorix Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Synorix Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a synorix: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Synorix command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &SynorixGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &SynorixGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &SynorixGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &SynorixGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &SynorixGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &SynorixGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &SynorixGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &SynorixGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &SynorixGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &SynorixGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &SynorixGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &SynorixGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &SynorixGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &SynorixGUI::showDebugWindow);
}

void SynorixGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void SynorixGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &SynorixGUI::setCurrentWalletBySelectorIndex);
...
void SynorixGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &SynorixGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &SynorixGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &SynorixGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &SynorixGUI::showProgress);
...
#ifdef ENABLE_WALLET
void SynorixGUI::enableHistoryAction(bool privacy)
{
...
void SynorixGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &SynorixGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &SynorixGUI::removeWallet);
...
WalletController* SynorixGUI::getWalletController()
{
    return m_wallet_controller;
}

void SynorixGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &SynorixGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &SynorixGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &SynorixGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &SynorixGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &SynorixGUI::incomingTransaction);
    connect(this, &SynorixGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
    wallet_view->setPrivacy(isPrivacyModeActivated());
    const QString display_name = walletModel->getDisplayName();
    m_wallet_selector->addItem(display_name, QVariant::fromValue(walletModel));
}

void SynorixGUI::removeWallet(WalletModel* walletModel)
{
...
void SynorixGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void SynorixGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void SynorixGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SynorixGUI::setWalletActionsEnabled(bool enabled)
{
...
void SynorixGUI::createTrayIcon()
{
...
void SynorixGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &SynorixGUI::toggleHidden);
...
void SynorixGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void SynorixGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void SynorixGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void SynorixGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void SynorixGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void SynorixGUI::openClicked()
{
...
void SynorixGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SynorixGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SynorixGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SynorixGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SynorixGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SynorixGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void SynorixGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void SynorixGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Synorix network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void SynorixGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SynorixGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void SynorixGUI::updateHeadersSyncProgressLabel()
{
...
void SynorixGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void SynorixGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &SynorixGUI::quitRequested);
...
void SynorixGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void SynorixGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &SynorixGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void SynorixGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void SynorixGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &SynorixGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &SynorixGUI::show);
                e->ignore();
            }
        }
    }
...
void SynorixGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SynorixGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SynorixGUI::incomingTransaction(const QString& date, SynorixUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(SynorixUnits::formatWithUnit(unit, amount, true));
...
void SynorixGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SynorixGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SynorixGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SynorixGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SynorixGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void SynorixGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SynorixGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void SynorixGUI::updateProxyIcon()
{
    std::string ip_port;
...
void SynorixGUI::updateWindowTitle()
{
...
void SynorixGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SynorixGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SynorixGUI::detectShutdown()
{
...
void SynorixGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void SynorixGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SynorixGUI* gui, const bilingual_str& message, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = SynorixGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void SynorixGUI::subscribeToCoreSignals()
{
...
void SynorixGUI::unsubscribeFromCoreSignals()
{
...
bool SynorixGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<SynorixUnit> units = SynorixUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const SynorixUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, SynorixUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const SynorixUnit u : SynorixUnits::availableUnits()) {
        menu->addAction(SynorixUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(SynorixUnit newUnits)
{
    setText(SynorixUnits::longName(newUnits));
}
