...
const QString BITCOIN_IPC_PREFIX("synorix:");
...
void PaymentServer::ipcParseCommandLine(int argc, char* argv[])
{
    for (int i = 1; i < argc; i++)
    {
        ...
        if (arg.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // synorix: URI
        {
    ...
    : QObject(parent)
{
    ...
    // on Mac: sent when you click synorix: links
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start synorix: click-to-pay handler"));
        }
        else {
...
//
// OSX-specific way of handling synorix: URIs
//
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("synorix://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'synorix://' is not a valid URI. Use 'synorix:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // synorix: URI
    {
        QUrlQuery uri((QUrl(s)));
        // normal URI
        {
            SendCoinsRecipient recipient;
            if (GUIUtil::parseSynorixURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Synorix address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
