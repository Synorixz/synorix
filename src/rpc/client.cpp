...
//! Specify whether parameter should be parsed by synorix-cli as a JSON value,
 ...
 *
 * JSON parameters need to be listed here to make synorix-cli parse command line
 * arguments as JSON, instead of passing them as raw strings. `JSON` and
 * `JSON_OR_STRING` formats both make `synorix-cli` attempt to parse the
 ...
 * `synorix-cli getblockstats <hash>` without needing to quote the hash string
 * as JSON (`'"<hash>"'`).
 *
 ...
 * to make synorix-cli treat them as positional parameters when `-named` is used.
 * This prevents `synorix-cli` from splitting strings like "my=wallet" into a named
 ...
 * other string parameters for that method need to be listed as well so synorix-cli
 ...
 *
 * For example, the command `synorix-cli -named createwallet "my=wallet"`,
