// Copyright (c) 2025-present The Synorix Core developers
...
Options:
  -m, --multiprocess     Run multiprocess binaries synorix-node, synorix-gui.
  -M, --monolithic       Run monolithic binaries synorixd, synorix-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'synorix-qt [ARGS]' or 'synorix-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'synorixd [ARGS]' or 'synorix-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'synorix-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'synorix-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'synorix-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_synorix [ARGS]'.
  chainstate [ARGS] Run synorix kernel chainstate util, equivalent to running 'synorix-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_synorix [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_synorix-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    ...
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "synorix-gui" : "synorix-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "synorix-node" : "synorixd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("synorix-cli");
            // Since "synorix rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("synorix-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("synorix-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_synorix");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("synorix-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_synorix");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_synorix-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("synorix-util");
        } else {
...
//! Execute the specified synorixd, synorix-qt or other command line in `args`
...
//!             be a relative path to a synorixd, synorix-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/synorix ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/synorix/synorix/pull/31375#discussion_r1861814807)
