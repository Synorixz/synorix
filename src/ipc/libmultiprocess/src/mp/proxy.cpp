// Copyright (c) The Synorix Core developers
...
thread_local ThreadContext g_thread_context; // NOLINT(synorix-nontrivial-threadlocal)
...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking synorix code so they have to run
    ...
    // functions run, so client objects are fully disconnected before synorix
    // code in the destructors are run. This way if the synorix code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In synorix core, running these callbacks in the right order is
