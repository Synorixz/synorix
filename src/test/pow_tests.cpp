...
BOOST_AUTO_TEST_CASE(get_next_work)
{
    ...
    // These cases exercise the legacy interval retarget and its transition
    // bounds. Mainnet runs the LWMA retarget now, so use a local copy of the
    // consensus params with LWMA disabled to keep testing the legacy path.
    Consensus::Params consensus = chainParams->GetConsensus();
    consensus.fLwmaPow = false;
...
BOOST_AUTO_TEST_CASE(get_next_work_pow_limit)
{
    const auto chainParams = CreateChainParams(*m_node.args, ChainType::MAIN);
    Consensus::Params consensus = chainParams->GetConsensus();
    consensus.fLwmaPow = false; // test legacy interval retarget
