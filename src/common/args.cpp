...
const char * const BITCOIN_CONF_FILENAME = "synorix.conf";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //synorix-tx using stdin
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Synorix
    //   new: C:\Users\Username\AppData\Local\Synorix
    // macOS: ~/Library/Application Support/Synorix
    // Unix-like: ~/.synorix
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Synorix";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Synorix";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Synorix";
#else
    // Unix-like
    return pathRet / ".synorix";
#endif
#endif
}
